...
export default function AdminBoard({ queues }: { queues: QueueItem[] }) {
  ...
  return (
    <div className="min-h-screen bg-base pb-24">
      ...
      <div className="flex items-center justify-between bg-gradient-to-br from-black via-accentDark to-accent px-4 py-5 shadow-lg">
        <h1 className="text-lg font-bold text-white">จัดการคิว</h1>
        <button
      ...
      <div className="mx-auto max-w-2xl px-4 pt-6">
        ...
        <form
          onSubmit={handleAdd}
          className="mb-6 flex gap-2 rounded-2xl bg-surface p-3 shadow-sm ring-1 ring-white/5"
        >
          <input
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="ชื่อลูกค้า (ไม่บังคับ)"
            className="flex-1 rounded-lg border border-white/10 bg-base px-3 py-3 text-base text-ink outline-none placeholder:text-stone-500 focus:border-accent focus:ring-2 focus:ring-accent/20"
          />
          <button
            type="submit"
            disabled={isPending}
            className="rounded-lg bg-accent px-5 py-3 text-base font-semibold text-white active:scale-[0.98] disabled:opacity-50"
          >
            + เพิ่มคิว
          </button>
        </form>
        ...
        <section className="mb-8">
          ...
          <div className="space-y-3">
            ...
            {active.map((q) => (
              <div
                key={q.id}
                className="rounded-2xl bg-surface p-4 shadow-sm ring-1 ring-white/5"
              >
                ...
                <div className="grid grid-cols-4 gap-2">
                  {STATUS_ORDER.map((s) => (
                    <button
                      ...
                      className={`rounded-lg py-3 text-xs font-semibold transition active:scale-95 disabled:opacity-40 ${
                        q.status === s
                          ? STATUS_STYLES[s] + " ring-2"
                          : "bg-base text-stone-400 ring-1 ring-white/10 hover:bg-white/5"
                      }`}
                    >
                      {STATUS_LABEL[s]}
                    </button>
